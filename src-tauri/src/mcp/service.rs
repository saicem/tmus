...
impl McpService {
    ...
    * `limit` - Optional limit to return only N apps. If not provided, defaults to 20.")]
    async fn get_uncategorized_apps(
        ...
        let uncategorized =
            category::get_uncategorized_apps(request)
                .await
                .map(|x| UncategorizedAppsResult {
                    apps: x
                        .apps
                        .into_iter()
                        .map(|app| AppSimple {
                            ...
                            description: app
                                .version
                                .as_ref()
                                .map(|v| v.file_description.clone().unwrap_or_default())
                                .unwrap_or_default(),
                            company: app
                                .version
                                .as_ref()
                                .map(|v| v.company_name.clone().unwrap_or_default())
                                .unwrap_or_default(),
                        })
                        .collect(),
                    total: x.total,
